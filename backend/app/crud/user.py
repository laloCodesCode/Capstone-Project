...
from backend.app.models import User, Listing
...
def get_user_by_school_email(db: Session, school_email: EmailStr) -> User | None:
...
def get_public_user_listings(db: Session, user_id: uuid.UUID) -> list[Listing]:
    return (
        db.query(Listing)
        .filter(
